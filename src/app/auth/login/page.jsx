...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      window.location.href = "../../panel/dashboard"; // Redirect to dashboard if already logged in
    }
  ...
  const handleLogin = async (e) => {
    ...
    if (error) {
      ...
      if (rememberMe) {
        localStorage.setItem("email", email); // Store email if "Remember Me" is checked
      } else {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#121212] overflow-x-hidden relative">
      ...
      <motion.nav
        ...
        transition={{ duration: 0.5 }}
        className="fixed mx-auto border-b-2 border-slate-700 top-0 left-0 right-0 z-20 bg-[#121212] bg-opacity-100 w-full"
      >
      ...
      <div className="flex justify-center items-center flex-grow container mx-auto px-4 py-8 z-10">
        ...
        >
          <h1 className="text-3xl font-extrabold mb-6 text-center text-blue-700">Login</h1>
          <form onSubmit={handleLogin}>
            ...
            <div className="mb-6 relative">
              ...
              <motion.input
                ...
                transition={{ type: "spring", stiffness: 300 }}
              />
              <div className="absolute top-9 right-3 cursor-pointer text-gray-500">
                {showPassword ? (
                  <EyeSlashIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7" onClick={() => setShowPassword(false)} />
                ) : (
                  <EyeIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7" onClick={() => setShowPassword(true)} />
                )}
              </div>
            </div>
