  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.sourceCodeLink && !/^(https?:\/\/)/i.test(formData.sourceCodeLink)) {
        throw new Error("Source code link must be a valid URL");
      }
      ...
      if (formData.thumbnail) {
        ...
        if (!data?.path) throw new Error("Thumbnail upload returned no path");
        const { data: publicUrlData } = supabase.storage
          .from("project-assets")
          .getPublicUrl(`thumbnails/${user.uid}/${fileName}`);
        if (!publicUrlData?.publicUrl) throw new Error("Failed to generate thumbnail public URL");
        thumbnailUrl = publicUrlData.publicUrl;
      }

      const previewImageUrls: string[] = [...formData.existingPreviewImageUrls];
      for (const image of formData.previewImages) {
        ...
        if (!data?.path) throw new Error("Preview image upload returned no path");
        const { data: publicUrlData } = supabase.storage
          .from("project-assets")
          .getPublicUrl(`previews/${user.uid}/${fileName}`);
        if (!publicUrlData?.publicUrl) throw new Error("Failed to generate preview image public URL");
        previewImageUrls.push(publicUrlData.publicUrl);
      }
      ...
      const previewImagesInput = document.getElementById("previewImages") as HTMLInputElement;
            ...
            >
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <Save className="w-5 h-5" aria-hidden="true" /> Add Project
                  </>
                )}
              </motion.button>
            </motion.div>
